...
function changeToBlack(img: Image, x: number, y: number): Color {
  img;
  x;
  y;
  return COLORS.BLACK;
...
function returnColorBlack(color: Color): Color {
  color;
  return COLORS.BLACK;
}

function returnFalseWithImgCoord(img: Image, x: number, y: number) {
  img;
  x;
  y;
  return false;
...
function returnTrueWithImgCoord(img: Image, x: number, y: number) {
  img;
  x;
  y;
  return true;
...
describe("mapWindow", () => {
  ...
  const changedImage2X2 = mapWindow(
    whiteImage,
    [0, 1], // Assumed to be a two element array containing [x_min, x_max]
    [0, 1], // Assumed to be a two element array containing [y_min, y_max]
    returnColorBlack
  );

  const changedImage1X1 = mapWindow(
    whiteImage,
    [0, 0], // Assumed to be a two element array containing [x_min, x_max]
